...
@interface DragOperation : NSObject {
    UIView* __unsafe_unretained draggable;                      // Item being dragged
    UIView<DragSource>* __unsafe_unretained dragSource;         // Item from which drag originated
    CGPoint initialPoint;                   // Point of initial touch
    
    UIView<DropTarget>* __unsafe_unretained currentDropTarget;  // Drop target the draggable item is hovering over (if any)
    
    BOOL dropAnimationInProgress;           // Whether a drop (or return to start) animation is in progress
}

@property (unsafe_unretained, nonatomic, readonly) UIView* draggable;
@property (unsafe_unretained, nonatomic, readonly) UIView<DragSource>* dragSource;
@property (nonatomic, readonly) CGPoint initialPoint;
@property (nonatomic, unsafe_unretained) UIView<DropTarget>* currentDropTarget;
